...
static int tas2562_set_slotwidth(struct tas2562_data *tas2562, int slot_width)
{
        int ret;
	switch (slot_width) {
        case 16:
			     ...
			     struct snd_soc_dai *dai)
{
	...
	ret = tas2562_set_slotwidth(tas2562, tas2562->rx_slot_len);
	if (ret) {
                dev_err(tas2562->dev, "set slotwidth failed, %d\n", ret);
                return ret;
        }
...
static int tas2562_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		...
		break;
	default:
		dev_err(tas2562->dev, "ASI format master is not found\n");
		ret = -EINVAL;
	}
...
static int tas256x_parse_dt(struct tas2562_data *tas2562)
{
	...
	ret = fwnode_property_read_u32(dev->fwnode, "ti,rx-slot-len",
			&tas2562->rx_slot_len);
	if (ret) {
		dev_info(dev, "Property %s is missing setting default TDM slot length\n",
			"ti,rx-slot-len");
		tas2562->rx_slot_len = 16;
	}
...
static const struct i2c_device_id tas256x_id[] = {
	{ "tas2562", TAS2562 },
	{ "tas2564", TAS2564 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, tas256x_id);

static int tas256x_probe(struct i2c_client *client)
{
	...
	id = i2c_match_id(tas256x_id, client);
	data->client = client;
	data->dev = &client->dev;
	data->model_id = id->driver_data;

	tas256x_parse_dt(data);
	...
	if (data->model_id == TAS2562)
		dev_info(dev, "TAS2562 chip found\n");

	if (data->model_id == TAS2564)
		dev_info(dev, "TAS2564 chip found\n");
...
static const struct of_device_id tas2562_of_match[] = {
	{ .compatible = "xiaomi,tas2562", },
	{ .compatible = "xiaomi,tas2564", },
	{ },
};
